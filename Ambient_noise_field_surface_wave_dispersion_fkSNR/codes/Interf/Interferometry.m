...
%       hugeDataFlag == 2 will also reduce FFT points during xcorr
...
interfmethod = procPar.interfmethod; % 'Correlation'
TWIN = procPar.TWIN; % Inf
tfpresent = procPar.tfpresent; % 'temporal'
interftimespan = procPar.interftimespan; % 'acausal+causal'
whiteNoise = procPar.whiteNoise; % 0.5
%
...
npairs = length(vsIndex_R);
%
Offset = zeros(npairs, 1);
Geometry = zeros(npairs, 2);
for i = 1 : npairs
    Geometry(i,:) = (AmbiSeis.Geometry(vsIndex_S(i),:) + AmbiSeis.Geometry(vsIndex_R(i),:))/2;
end
%
for i = 1 : npairs
...
% close(hwait)
%
interf_tAxes = interf_tAxes(tIndex);
