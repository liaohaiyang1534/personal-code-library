...
def write_stream_to_segy_2d(stream, output_file_path, input_file_path):
    ...
    with segyio.open(input_file_path, 'r', ignore_geometry=True) as src:
        # Define specifications for the new SEG-Y file based on the source
        spec = segyio.spec()
        ...
        # Create a new SEG-Y file to write data to
        with segyio.create(output_file_path, spec) as dst:
            # Copy headers from the source file to the destination
            dst.text[0] = src.text[0]
            dst.bin.update(src.bin)
            
            # Iterate over traces in the stream and write them to the new file
            for i, tr in enumerate(stream):
                # Copy trace header from source, then write trace data
