...
def process_sgy(file_path, output_folder, minus_min, minus_second, duration):
    ...
    with segyio.open(file_path, "r", ignore_geometry=True) as f:
        ...
        for i in range(trace_count):
            trace = f.trace[i]
            sta_lta = classic_sta_lta(trace, nsta, nlta)
    ...
    if cleaned_trigger_times:
        ...
        for time in cleaned_trigger_times:
            ...
            copy_time_range(file_path, time, duration, prefix='_output', output_folder=output_folder, minus_min=minus_min, minus_second=minus_second)
    else:
        print("No trigger found above the threshold.")

def copy_time_range(src_filename, start_time, duration=1.0, prefix='_output', output_folder='', minus_min=0, minus_second=0):
    start_time -= 0.01
    if start_time < 0:
        start_time = 0
    ...
    with segyio.open(src_filename, "r", ignore_geometry=True) as src:
        ...
        output_data = np.zeros((src.tracecount, expected_samples))
        ...
        spec.samples = np.arange(0, expected_samples * sample_interval, sample_interval)
        ...
        base_time = base_time - timedelta(minutes=minus_min, seconds=minus_second)
        trigger_time = base_time + timedelta(seconds=round(start_time, 3))
        trigger_time_str = trigger_time.strftime('%Y-%m-%d-%H-%M-%S.%f')[:-3]

        dst_filename = f"{trigger_time_str}{prefix}{src_extension}"
...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        print("Usage: python stalta.py <input_sgy_file_path> <output_folder_path> <minus_min> <minus_second> <duration>")
        sys.exit(1)
    
    input_sgy_file_path = sys.argv[1]
    output_folder_path = sys.argv[2]
    minus_min = int(sys.argv[3])
    minus_second = int(sys.argv[4])
    duration = float(sys.argv[5])

    process_sgy(input_sgy_file_path, output_folder_path, minus_min, minus_second, duration)
